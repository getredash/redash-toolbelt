...
def test_group_list():
    ...
    assert_in("admin", run("group", "list").stdout)
    assert_in("default", run("group", "list").stdout)
    assert_is_instance(
...
def test_group_completion():
    eq_(run_completion(["group", "open"], ""), ["1", "2"])
    eq_(run_completion(["group", "open"], "de"), ["2"])
    eq_(run_completion(["group", "open"], "min"), ["1"])
